...
class Phone extends Product {
  ...
  showAllInformation() {
    console.log(
      `Its Phone, name = ${this.name}  sale= ${this.sale} price= ${this.price} memory= ${this.memory} version=${this.version}`
    );
  }
  set memory(memory) {
  ...
  showAllInformation(count) {
    if (count) {
      console.log(
        ` name = ${
          this.name
        }  total price = ${this.countTotalPrice()} material= ${
          this.material
        } color=${this.color}`
      );
    } else {
      console.log(
        ` name = ${this.name}  sale= ${this.sale} price= ${this.price} material= ${this.material} color=${this.color}`
      );
    }
  }
}
